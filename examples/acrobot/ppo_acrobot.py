...
class MyAcroBot:
    ...
    def _normalize_obs(self, obs):
        ...
        obs[-2:] = 10*((obs[-2:] - self.low[-2:])/(self.high[-2:]-self.low[-2:]) - 0.5)
        #print(obs)
    ...
    def reset(self):
        ...
        return self._normalize_obs(state), info

    def step(self, action):

        state, reward, terminated, truncated, info = self.env.step(action)
        return self._normalize_obs(state), reward, terminated, truncated, info
...
def plot_eval_results(scores_list):
    ...
    ax.set_ylabel("Score")
    plt.savefig("ppo_acrobot_eval.jpg")
...
if __name__ == "__main__":
    ...
    sampler = SequentialSampler(32, MyAcroBot, env_info, PPOPolicy, policy_d, 500)
    learner = PPOLearner(0.99, policy_d)
    learner.set_training_params(lamb=0.94, minibatch_size=64, epochs=4, lr=0.0003, vf_loss_coeff=0.5)
    ...
    for i in range(steps):
        ...
        scores = []
        for j in range(16):
            batch = sampler.evaluate_policy(1, weights, 500)
            done_at = np.where(np.squeeze(batch["dones"], axis=2)>=1.0)[1][0]
            scores.append(-(done_at+1))
        mean = np.mean(scores)
